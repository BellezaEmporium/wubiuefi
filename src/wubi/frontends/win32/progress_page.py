...
class ProgressPage(Page):
    ...
    def on_init(self):
        ...
        self.insert_header(
            _("Installing %(distro)s-%(version)s") % dict(
                distro=distro_name,
        ...
        self.main.localiso_button = ui.Button(
            self.main, 20, 150, 200, 20, _("Do not download, use a local file")
        )
    ...
    def on_progress(self, task, message=None):
        ...
        if task.get_percent_completed() > 0:
            ...
            self.main.subtask_label.set_text(
                _("Remaining time approximately %s") % task.estimate_remaining_time()[0]
            )
