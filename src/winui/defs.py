...
HCURSOR = getattr(wintypes, "HCURSOR", HICON)
...
class WNDCLASSEX(ctypes.Structure):
    ...
    def __init__(self,
        ...
        if not icon:
            icon = ctypes.windll.user32.LoadIconW(None, ctypes.c_wchar_p(IDI_APPLICATION))
        if not iconsm:
            iconsm = icon
        if not cursor:
            cursor = ctypes.windll.user32.LoadCursorW(None, ctypes.c_wchar_p(IDC_ARROW))
        if not background:
            background = COLOR_WINDOW + 1
            # or old fallback:
            # ctypes.windll.gdi32.GetStockObject(WHITE_BRUSH)
